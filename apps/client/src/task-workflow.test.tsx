...
const inbox = { id: '11111111-1111-4111-8111-111111111111', title: 'Inbox', folder_id: null, is_inbox: true, position: 0, version: 1, is_trashed: false, trashed_at: null, default_column: { id: '22222222-2222-4222-8222-222222222222', is_default: true } };
const work = { ...inbox, id: '55555555-5555-4555-8555-555555555555', title: 'Work', is_inbox: false, version: 2, default_column: { id: '66666666-6666-4666-8666-666666666666', is_default: true } };
const workTask = { id: '77777777-7777-4777-8777-777777777777', title: 'First task', status: 'todo', version: 3, column_id: work.default_column.id, owner_user_id: '44444444-4444-4444-8444-444444444444', created_by_user_id: '44444444-4444-4444-8444-444444444444', source: { platform: 'dotick', external_account_id: null, external_id: null }, created_at: '2026-09-19T06:00:00Z', updated_at: '2026-09-19T06:00:00Z' };
...
function openWork() { fireEvent.click(screen.getByRole('button', { name: 'Open Work' })); }
...
test('creates a Task in the selected List destination with a client operation ID', async () => {
  ...
  const created = { ...workTask, id: '88888888-8888-4888-8888-888888888888', title: 'New task', version: 1 };
  const fetch = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (url.endsWith('/api/v1/tasks') && init?.method === 'POST') { taskCreateBody = JSON.parse(String(init.body)) as Record<string, unknown>; return json(created, 201); }
    if (url.endsWith('/api/v1/tasks')) return json({ results: [workTask] });
    throw new Error(`Unhandled request: ${url}`);
  });
  vi.stubGlobal('fetch', fetch);
  render(<App />); signIn(); await screen.findByRole('heading', { name: 'Inbox' }); openWork();
  ...
  expect(taskCreateBody).toEqual({ title: 'New task', column_id: work.default_column.id, operation_id: 'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa' });
...
test('preserves a failed Task draft and operation ID for retry', async () => {
  ...
  const fetch = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (url.endsWith('/api/v1/account/bootstrap')) return json({ preferences: { timezone: 'Europe/London' }, inbox });
    if (url.endsWith('/api/v1/lists')) return json({ results: [inbox, work] });
    if (url.endsWith('/api/v1/tasks') && init?.method === 'POST') { bodies.push(JSON.parse(String(init.body)) as Record<string, unknown>); posts += 1; if (posts === 1) throw new TypeError('offline'); return json(created, 201); }
...
test('edits Task title and status using the latest returned Task version', async () => {
  ...
  expect(patchBodies).toEqual([{ version: 3, title: 'Updated task' }, { version: 4, status: 'done' }]);
});
...
test("supports Todo, Done, and Won't do status controls", async () => {
  ...
  const fetch = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (url.endsWith(`/api/v1/tasks/${workTask.id}`) && init?.method === 'PATCH') {
      const body = JSON.parse(String(init.body)) as { version: number; status: 'todo' | 'done' | 'wont_do' };
  ...
  vi.stubGlobal('fetch', fetch);
  render(<App />); signIn(); await screen.findByRole('heading', { name: 'Inbox' }); openWork(); await screen.findByText('First task');
