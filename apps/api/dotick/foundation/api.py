...
class FoundationView(APIView):
    # Temporary developer-only transport. I1 owns product JWT/OAuth/Passkey auth.
...
class CheckpointList(FoundationView):
    ...
    def post(self, request):
        data = CheckpointInput(data=request.data)
        data.is_valid(raise_exception=True)
        row = application.create_checkpoint(actor_id=request.user.id, **data.validated_data)
        return Response(CheckpointOutput(row).data, status=201)


class CheckpointDetail(FoundationView):
    def get(self, request, checkpoint_id):
        row = application.get_checkpoint(actor_id=request.user.id, checkpoint_id=checkpoint_id)
